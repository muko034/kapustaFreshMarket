...
//        CFG.Draw.text(CFG.beforeTime, 10, 30, 20,"0");
    ...
    drawBackground: function(){
        CFG.ctx.drawImage(CFG.sprite, 0, 0, 480, 720, 0, 0, 320, 480);
    },
...
CFG.View.menu.paint = function(){
    CFG.ctx.drawImage(CFG.sprite, 480, 0, 320, 480, 0, 0, 320, 480);
    if (CFG.isLoggedIn){
...
CFG.quaterMidpointCircle = function(radius){
    var circlePoints = new Array();
    var x, y, d, deltaE, deltaSE;
    x = 0;
    y = radius;
    d = 1-radius;
    deltaE = 3;
...
CFG.getCircleBound = function(radius) {
    var circlePoints = CFG.quaterMidpointCircle(radius);
...
CFG.Field = function(){
    ...
    this.isPointIn = function(point){
        var x = Math.abs(Math.round(point.x-140));
        var y = Math.round(point.y);
        return (point.x >= 40 && point.x < 300 &&
                point.y >= -CFG.Field.BOUND[x]+220 && point.y < CFG.Field.BOUND[x]+220);
    };
...
CFG.Field.setBound = function(){
    return CFG.getCircleBound(140);
};

CFG.Field.CAPABILITY = 25000;
CFG.Field.BOUND = CFG.Field.setBound();
...
//            if (this.isSpraying){
//                CFG.ctx.drawImage(CFG.sprite, 480, 480, 100, 135, this.x, this.y-15, 50, 68);
//            } else {
                CFG.ctx.drawImage(CFG.sprite, 480, 480, 100, 150, this.x, this.y, 50, 75);
//            }
//        };
...
CFG.Sprayer.setSplatterBound = function(){
    return CFG.getCircleBound(CFG.Sprayer.RADIUS);
};
...
CFG.Splatter = function(splatter){
    ...
    this.render = function(){
        ...
        for (var i=0; i < this.points.length; i++){
            p  = this.points[i];
            CFG.Draw.rect(p.x, p.y, 2, 2, "rgba(255,103,255," + this.opacity + ")");
        }
    };
};
