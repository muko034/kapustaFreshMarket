...
//        $("canvas").css("margin-top", Math.round((window.innerHeight - CFG.currentHeight)/2) + 'px');
...
//        CFG.Draw.text(CFG.beforeTime, 10, 30, 20,"0");
    ...
    drawBackground: function(){
        CFG.ctx.drawImage(CFG.sprite, 0, 0, 900, 600, 0, 0, 480, 320);
    },
...
CFG.View.menu.init = function(){
    ...
    this.xButton = {"id": "xButton", "type": "button", "visible": false,
                    "left": 440, "top": 20, "width": 20, "height": 20};
...
CFG.View.menu.paint = function(){
    CFG.ctx.drawImage(CFG.sprite, 0, 600, 480, 320, 0, 0, 480, 320);
    if (CFG.isLoggedIn){
...
CFG.Sprayer = function(){
    ...
    this.spray = function() {
        ...
        var p, a = CFG.Sprayer.HALF_SIDE_LENGTH;
        var x, y, splatterBound = CFG.Sprayer.SPLATTER_BOUND;
        for (var i=0; i < CFG.Sprayer.NUMBER_OF_DOTS; ++i){
            p = new CFG.Point();
            x = Math.random() * a * 2 - a;  // x = -HALF_SIDE_LENGTH,...,HALF_SIDE_LENGTH
            x = Math.round(x);
            p.x = x + this.x;
            x = Math.abs(x);
            y = Math.random() * (a - splatterBound[x]) + splatterBound[x];  // y = -HALF_SIDE_LENGTH[x], ..., SPLATTER_BOUND[x]
            y = Math.round(y);
            p.y = y + this.y;
            spatter.push(p);
        }
...
//        this.img.onload = function(){
            if (CFG.Input.left){
                CFG.ctx.drawImage(CFG.sprite, 680, 600, 150, 75, this.x-25, this.y-60, 75, 35);
            } else {
                CFG.ctx.drawImage(CFG.sprite, 680, 675, 150, 75, this.x-50, this.y-60, 75, 35);
            }
//        };
...
CFG.Sprayer.HALF_SIDE_LENGTH = 35;
CFG.Sprayer.NUMBER_OF_DOTS = 100;
CFG.Sprayer.SPRAY_DELAY = 6; // 6 * TICKS = 6 * (1000/60) = 100ms


CFG.Sprayer.bresenhamLine = function(){
       
    // zmienne pomocnicze
    var linePoints = new Array(); 
    var d, dx, dy, ai, bi, xi, yi;
    var x1=0, y1 = -CFG.Sprayer.HALF_SIDE_LENGTH;
    var x2 = CFG.Sprayer.HALF_SIDE_LENGTH, y2 = CFG.Sprayer.HALF_SIDE_LENGTH;
    var x = x1, y = y1;
    ...
    // pierwszy piksel
    linePoints.push(new CFG.Point(x,y));
    // oś wiodąca OX
    if (dx > dy)
    {
        ai = (dy - dx) * 2;
...
CFG.Sprayer.setSplatterBound = function(){
    var linePoints = CFG.Sprayer.bresenhamLine();
    

    var bound = new Array();
    var prevX = 0;
    var maxY = CFG.Sprayer.HALF_SIDE_LENGTH;
    for (var i=0; i < linePoints.length; i++)
    {
        bound;
        if (linePoints[i].x === prevX)
        {
            if (linePoints[i].y > maxY) maxY = linePoints[i].y;
        }
       else
        {
            bound.push(maxY);
        ...
        prevX = linePoints[i].x;
    }
    bound.push(maxY);
    linePoints.splice(0); // == .clear();
    linePoints = null;    //  it can be garbage colected
    return bound;
};
...
CFG.Splatter = function(splatter){
    ...
    this.render = function(){
        ...
        for (var i=0; i < this.points.length; i++){
            p  = this.points[i];
            CFG.Draw.rect(p.x, p.y, 2, 2, "rgba(255,66,66," + this.opacity + ")");
        }
    };
};
...
//window.addEventListener('load', CFG.Sprayer.setSplatterBound(), false);
